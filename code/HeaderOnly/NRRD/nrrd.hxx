...
namespace NRRD
{
	...
	inline const std::map<std::string,std::string>& known_types()
	{
		...
		std::map<std::string,std::string> hdr_fields=std::map<std::string,std::string>())
	{
		...
		// Other c++ types are written as block.
		if (known_types().find(hdr_fields["type"])==known_types().end()) {
			hdr_keys["typename"]=hdr_fields["type"];
			hdr_fields["type"]="block";
			hdr_fields["block size"]=toString(sizeof(T));
		}
		// Dimension and size
		...
		std::map<std::string,std::string>* hdr_keys=0x0)
	{
		...
		file.close();
		if (hdr_fields["type"]!="block")
		{
			// Map all possible NRRD-types to the corresponding C-type to avoid confusion.
			auto t=known_types().find(hdr_fields["type"]);
			if (t==known_types().end())
			{
		...
		std::map<std::string,std::string>* hdr_fields=0x0)
	{
		...
		// For type "block" we just verify that size of type T in bytes matches the block size.
		if ((*hdr_fields)["type"]=="block")
		{
			int block_size=0;
			if (hdr_fields->find("block size")!=hdr_fields->end()) block_size=stringTo<int>((*hdr_fields)["block size"]);
			if (hdr_fields->find("blocksize" )!=hdr_fields->end()) block_size=stringTo<int>((*hdr_fields)["blocksize"] );
			if (sizeof(T)!=block_size) {
				std::cerr << "NDDR::load<T>(...): Field \"block size\" not found or size mismatch ("<< block_size << "/" << sizeof(T) << ")\n";
				return false;
			}
		}

		// Make sure the file type matches 
		if ((*hdr_fields)["type"]!=typeName<T>() && (*hdr_fields)["type"]!="block")
		{
			int n=1;
			if ((*hdr_fields)["type"]=="")
			{

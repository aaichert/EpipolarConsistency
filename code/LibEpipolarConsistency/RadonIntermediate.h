...
namespace EpipolarConsistency
{
	...
	public:
		...
		// Update Radon intermediate data with CPU memory
		...
		/// Readback Radon Intermediate data to CPU. If gpu_memory_only is set, the texture will be transferred to global GPU memory but not read back to RAM.
		void readback(bool gpu_memory_only=false);

		/// Access to the size of the Radon transform. 0:distance 1:angle
		int getNumberOfRadonBins(int dim) const;
		...
		double getStepRadonBinning(int dim=1) const;

		/// Copy to texture and get rid of plain array. After this, readback(...) will no longer work. (Minor Fixme)
		UtilsCuda::BindlessTexture2D<float>* getTexture();

		/// Access to raw data on CPU (may return invalid image). See also: readback(...), getTexture().
		...
		inline float sample(Line& line)
		{
			...
			line[0]=atan2f(line[1],-line[0])/(float)Pi+1.0f;
		...
		inline float tex2D(float s, float  t) { return (float)m_raw_cpu(n_alpha*s,n_t*t); }

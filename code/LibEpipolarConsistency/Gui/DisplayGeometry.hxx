...
namespace EpipolarConsistency
{
	...
	inline UtilsQt::Figure displayInputData(const std::vector<Geometry::ProjectionMatrix>& Ps, const std::vector<EpipolarConsistency::RadonIntermediate*>& dtrs, double spacing, const std::string& figure_name="Input Data")
	{
		...
		int n_alpha=dtrs.front()->getStepRadonBinning(0);
		int n_t    =dtrs.front()->getStepRadonBinning(1);
	...
	inline void displayRadonSamples(int r, int i, double dkappa, const std::vector<Geometry::ProjectionMatrix>& Ps,const std::vector<EpipolarConsistency::RadonIntermediate*>& dtrs, GetSetGui::ProgressInterface* progress=0x0, const std::string& figure_name="Radon Samples")
	{
		using UtilsQt::Figure;
		...
		dtrs[r]->readback();
		dtrs[i]->readback();
		if (!figure_r.exists(true)) figure_r.setImage(dtrs[r]->data(),0,0,true);
		if (!figure_i.exists(true)) figure_i.setImage(dtrs[i]->data(),0,0,true);
		...
		E0/=E0.head(3).norm();
		E90/=E90.head(3).norm();
		// Transformation to move lines by half image size (because Radon transform assuems coordinate origin in center of image.)
		auto HinvT=Translation(-.5*dtrs[r]->getOriginalImageSize(0),-.5*dtrs[r]->getOriginalImageSize(1)).inverse().transpose().eval();
		...
		for (double kappa=-0.5*Pi;kappa<0.5*Pi;kappa+=dkappa)
		{
			...
			l0_kappa/=l0_kappa.head(2).norm(); // remove - just for debugging
			l1_kappa/=l1_kappa.head(2).norm(); // remove - just for debugging
			// Move coordinate origin to image center.
			l0_kappa=HinvT*l0_kappa;
			l1_kappa=HinvT*l1_kappa;
			...
			// Account for periodicity of the Radon transform
			s_kappa.flip0=s_kappa.alpha0>Pi;
			s_kappa.flip1=s_kappa.alpha1>Pi;
			if (s_kappa.flip0) s_kappa.t0*=-1;
			if (s_kappa.flip1) s_kappa.t1*=-1;
			if (s_kappa.flip0) s_kappa.alpha0-=Pi;
			if (s_kappa.flip1) s_kappa.alpha1-=Pi;
			// Compute Radon bins
			s_kappa.bin_alpha0=s_kappa.alpha0/Pi*dtrs[r]->getNumberOfRadonBins(0);
			s_kappa.bin_alpha1=s_kappa.alpha1/Pi*dtrs[i]->getNumberOfRadonBins(0);
			s_kappa.bin_t0    =s_kappa.t0    /dtrs[r]->getStepRadonBinning(1)+dtrs[r]->getNumberOfRadonBins(1)/2;
			s_kappa.bin_t1    =s_kappa.t1    /dtrs[i]->getStepRadonBinning(1)+dtrs[i]->getNumberOfRadonBins(1)/2;
			// Ignore Samples outside of reference Radon transform
			if (s_kappa.bin_t0<0 || s_kappa.bin_t0>=dtrs[r]->getNumberOfRadonBins(1)
			||  s_kappa.bin_t1<0 || s_kappa.bin_t1>=dtrs[i]->getNumberOfRadonBins(1))
				continue;
			...
			if (s_kappa.flip0) s_kappa.v0*=-1;
			if (s_kappa.flip1) s_kappa.v1*=-1;
		...
		{
			auto& radon_samples_r=figure_r.overlay().set(toString(i), GraphicsItems::Collection<GraphicsItems::Line2D>());
			radon_samples_r.items.reserve(radon_bins_t0_by_alpha0.size());
			auto it     =radon_bins_t0_by_alpha0.begin();
			auto it_plus=radon_bins_t0_by_alpha0.begin();
			++it_plus;
			for (;it_plus!=radon_bins_t0_by_alpha0.end();++it,++it_plus)
				radon_samples_r.items.push_back(GraphicsItems::Line2D(it_plus->first,it_plus->second,it->first,it->second,1,color_i));
		}
		{
		...
		plot.graph().setData(kappas,v0s).setName(std::string("Radon Intermediate ")+toString(r)).setColor(GraphicsItems::colorByIndex(r));
		plot.graph().setData(kappas,v1s).setName(std::string("Radon Intermediate ")+toString(i)).setColor(GraphicsItems::colorByIndex(i));

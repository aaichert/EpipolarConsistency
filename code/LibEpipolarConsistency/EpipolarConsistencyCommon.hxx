...
template <typename T, int M, int O, int N>
__device__ __host__ inline void xgemm(const T *A, const T *B, T *C)
{
	for (int i = 0; i < M; i++)
		for (int j = 0; j < N; j++)
		{
...
__device__ __host__ inline void lineToSampleDtr(float *line, float range_t)
{
	...
	line[0] = atan2f(line[1], -line[0]) / Pi + 1.0f;

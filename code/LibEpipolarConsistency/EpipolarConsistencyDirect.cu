		...
		float *out)
{
	...
	if (fbcc_d)
	{
		...
		for (float t=t_min; t<=t_max; t+=step)
		{
			...
			// Compute weight as derivative of transform over the distance of the virtual pixel to the source.
			float fbcc_weight=fbcc.phi.derivative(t)/sqrtf(u_prime*u_prime+fbcc.d_l_kappa_C_sq);
			// Finally accumulate weighted sum.
			sum+=step*tex2D<float>(tex,o[0]+t*d[0],o[1]+t*d[1])*fbcc_weight;
		}
		out[idx]=sum;
	}
	else
	{
		// Usual ECC. Numeric differenciation in direction l[0],l[1] with half pixel distance in either direction.
		l[0]*=0.5f;
		l[1]*=0.5f;
		...
		for (float t=t_min; t<=t_max; t+=step)
		{
			float u=o[0]+t*d[0];
			float v=o[1]+t*d[1];
			sump+=tex2D<float>(tex,u+l[0],v+l[1])*step;
			summ+=tex2D<float>(tex,u-l[0],v-l[1])*step;
		}

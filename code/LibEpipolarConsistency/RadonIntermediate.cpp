...
RadonIntermediate::RadonIntermediate(const NRRD::ImageView<float> &projectionData, int size_alpha, int size_t, bool computeDerivative)
	: m_bin_size_angle(0), m_bin_size_distance(0), m_is_derivative(0), n_x(0), n_y(0), n_alpha(size_alpha), n_t(size_t), m_tex(0x0)
{
...
RadonIntermediate::RadonIntermediate(const std::string path)
	: m_bin_size_angle(0), m_bin_size_distance(0), m_is_derivative(0), n_x(0), n_y(0), n_alpha(0), n_t(0), m_tex(0x0), m_raw_gpu(0x0)
{
	m_raw_cpu.load(path);
	if (!m_raw_cpu)
	{
...
RadonIntermediate::RadonIntermediate(const NRRD::ImageView<float> &radon_intermediate_image)
	: m_bin_size_angle(0), m_bin_size_distance(0), m_is_derivative(0), n_x(0), n_y(0), n_alpha(0), n_t(0), m_tex(0x0)
{
...
int RadonIntermediate::getNumberOfRadonBins(int dim) const
{
	if (dim)
		return n_t;
	else
...
double RadonIntermediate::getStepRadonBinning(int dim) const
{
	if (dim)
		return m_bin_size_distance;
	else
		return m_bin_size_angle;
}
